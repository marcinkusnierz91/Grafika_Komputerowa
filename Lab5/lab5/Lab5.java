...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
	...
	private void korkociag(GL2 gl2, int n) {
		...
		float pointSize = 1 + i / 10.0f;
		float x = (float) (2 * Math.cos(i * 2 * Math.PI / 20));
		float y = (float) (2 * Math.sin(i * 2 * Math.PI / 20));
		float z = 0.05f * i;
	...
	private void piramida(GL2 gl2, int n) {
		...
		for (int i = 0; i < n; i++) {
		    float j = i - 1;
		    float k = i;
		    float angleJ = j * 2 * (float) Math.PI / n;
		    float angleK = k * 2 * (float) Math.PI / n;

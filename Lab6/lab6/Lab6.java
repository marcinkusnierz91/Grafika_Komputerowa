...
public class Lab6 extends GLJPanel implements GLEventListener  {
	...
	private void piramida(GL2 gl2) {
		...
		for (int i = 0; i < 4; i++) {
			float j = i - 1;
			float k = i;
			float angleJ = j * 2 * (float) Math.PI / 4;
			float angleK = k * 2 * (float) Math.PI / 4;
			gl2.glBegin(GL2.GL_TRIANGLE_FAN);
			gl2.glVertex3f((float) Math.cos(angleJ), (float) Math.sin(angleJ), 4f);
			gl2.glVertex3f((float) Math.cos(angleK), (float) Math.sin(angleK), 4f);
	...
	public void init(GLAutoDrawable drawable) {
		...
		float[] diffuseColor = {0.8f, 0.8f, 0.2f, 1};    // Yellow diffuse light
	    float[] specularColor = {0.2f, 0.2f, 0.8f, 1};   // Blue specular light
	    float[] ambientColor = {0.2f, 0.2f, 0.2f, 1};    // Dark gray ambient light
	    float[] position = {1, 1, 1, 0};                 // Position the light at (1, 1, 1)

	    gl2.glLightfv(GL2.GL_LIGHT1, GL2.GL_DIFFUSE, diffuseColor, 0);
	    gl2.glLightfv(GL2.GL_LIGHT1, GL2.GL_SPECULAR, specularColor, 0);
	    gl2.glLightfv(GL2.GL_LIGHT1, GL2.GL_AMBIENT, ambientColor, 0);
